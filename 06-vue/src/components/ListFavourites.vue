...
const removeMeal = (meal: string) => {
  favouriteStore.removeFromFavourite({ name: meal })
}
...
<template>
  <div class="flex flex-col gap-y-5 border p-4 rounded">
    <div class="text-lg font-medium text-gray-900 dark:text-white">Favourite Meals</div>
    <div v-for="meal in favouriteStore.meals" :key="meal">
